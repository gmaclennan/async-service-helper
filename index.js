...
const kSetState = Symbol("setState");

/** @typedef {import('./lib/shared-types').AsyncServiceState} AsyncServiceState */

/**
 * @typedef {Object} InternalEvents
 * @property {() => void} started
 * @property {() => void} stopped
 * @property {(error: Error) => void} internalError
...
class AsyncService extends TypedEmitter {
  ...
  [kSetState](state) {
    ...
    else if (state.value === "error") this.#emitter.emit("internalError", state.error);
  ...
  async started() {
    ...
    return new Promise((resolve, reject) => {
      ...
      emitter.once("internalError", onError);
      function onStarted() {
        emitter.off("internalError", onError);
        resolve();
      }
      /** @param {Error} err */
  ...
  async start(...args) {
    ...
    try {
      this[kSetState]({ value: "starting" });
      await this._start.apply(this, args);
      this[kSetState]({ value: "started" });
    } catch (e) {
      this[kSetState]({ value: "error", error: e });
      throw e;
    }
  }
  ...
  async stop() {
    ...
    try {
      this[kSetState]({ value: "stopping" });
      await this._stop();
      this[kSetState]({ value: "stopped" });
    } catch (e) {
      this[kSetState]({ value: "error", error: e });
      throw e;
    }
